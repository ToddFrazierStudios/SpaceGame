...
public class ControllerProfile {
    ...
    private ControllerProfile(Dictionary<string, string> tab, string[] nams, bool osx, bool dpad, bool invRS, Platform plat) {
        table = tab;
        names = nams;
    ...
    private static void loadNonBundledProfiles() {
        ...
        foreach (string file in files) {
            parseJSON(File.ReadAllText(file));
        }
    }
    ...
    private static void parseJSON(string json) {
        ...
        profiles.Add(new ControllerProfile(table, names, osxTriggerCompNeeded, analogDPad, invertedRightStick, platform));
    }
}