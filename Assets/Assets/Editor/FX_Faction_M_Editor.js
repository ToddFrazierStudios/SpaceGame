...
var FoldOut2 : boolean;
	...
	EditorGUILayout.LabelField("ForceX Faction Manager: Version 1.0.3a", EditorStyles.boldLabel);
...
function FactionRelations(){
	...
	if(target.Initalize){
		...
		EditorGUILayout.Space ();
		EditorGUILayout.LabelField("IFF Ranges :", EditorStyles.boldLabel);
		EditorGUILayout.BeginVertical ("HelpBox");
		EditorGUILayout.Space ();	
		EditorGUILayout.LabelField ("Ranges -1000 --> 1000");
		EditorGUILayout.Space ();
		target.HFS[0] = EditorGUILayout.IntSlider("Hostile Value < ", target.HFS[0], -1000, 0);
		target.HFS[1] = EditorGUILayout.IntSlider("Friendly Value > ", target.HFS[1],0, 1000);

		var h : float = target.HFS[0];
		var f : float = target.HFS[1];
		...
		EditorGUILayout.MinMaxSlider(h, f, -1000.0, 1000.0);
		
		target.HFS[0] = Mathf.RoundToInt(h);
		target.HFS[1] = Mathf.RoundToInt(f);
		...
		EditorGUILayout.BeginVertical ("HelpBox");
		EditorGUILayout.Space ();
		target.PlayerFaction = EditorGUILayout.Slider(target.Factions[target.PlayerFaction] ,target.PlayerFaction, 0, target.Factions.Length - 1);
		EditorGUILayout.Space ();	
		EditorGUILayout.EndVertical ();
		EditorGUILayout.Space ();
...
if(FoldOut1){
	...
	for (i = 0; i < target.PlayerRelations.Length; i++){//display  all the elements of the array
		EditorGUILayout.Space ();
		target.PlayerRelations[i] = EditorGUILayout.Slider("Player  <---->  " + target.Factions[i].ToString(),target.PlayerRelations[i], -1000, 1000);
	}
}
EditorGUILayout.Space ();
EditorGUILayout.EndVertical ();
	...
	if(FoldOut2){		
		...
		for(var n : int = 0; n < TempFaction.Length; n++){
			for(var x : int = 0; x < ((NumberOfFactions - 1) - cnt[2]); x++){
				EditorGUILayout.Space ();
				EditorGUILayout.LabelField(target.Factions[cnt[2]].ToString() + "  <---->  " + target.Factions[((cnt[2] + cnt[1]) + 1)]);
				target.StartRelations[cnt[0]] = EditorGUILayout.Slider( "Faction Relation: ",target.StartRelations[cnt[0]], -1000, 1000);
